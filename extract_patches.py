...
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ...
    for i in range(len(test_masks)):
        for x in range(len(test_masks[i])):
            for y in range(len(test_masks[i,x])):
                for z in range(len(test_masks[i,x,y])):
                    if test_masks[i,x,y,z] != 0:
                        test_masks[i,x,y,z] = 1
    #-----------
    #extend both images and masks so they can be divided exactly by the patches dimensions
    test_imgs = test_imgs[0:Imgs_to_test,:,:,:]
    test_masks = test_masks[0:Imgs_to_test,:,:,:]
    ...
    #check masks are within 0-1
    #TODO: mask values are eigher 0 or 0.98431373.... not 1 -> skip this
    #value of high value is 250 instead of 255...if you look at it with MS Paint it is 255!
...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    ...
    print("extract random: ")
    print("n_patches: " + str(N_patches))
    print("full imgs shape: " + str(full_imgs.shape[0]))
